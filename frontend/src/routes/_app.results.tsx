...
function Results() {
  ...
  useEffect(() => {
    api.get<{ documents: DocResult[] }>("/documents/history")
    ...
    <div className="mx-auto max-w-6xl space-y-6 animate-fade-in">
      ...
      <Card>
        <CardContent className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4">
          <Info label="Document" value={doc.name} />
          <Info label="Original language" value={doc.source} />
          <Info label="Translated to" value={doc.target} />
          <div>
            <div className="text-xs text-muted-foreground font-medium">Quality score</div>
            <div className="mt-2 flex items-center gap-2">
              <Progress value={doc.qualityScore ?? 0} className="h-2" />
              <span className="text-sm font-semibold">{doc.qualityScore ?? 0}%</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {doc.processingResult?.summary && (
          <ResultPanel
            title="AI Summary"
            badge={`${doc.source}`}
            body={doc.processingResult.summary}
          />
        )}
        {doc.processingResult?.translation && (
          <ResultPanel
            title="Translation"
            badge={doc.target}
            body={doc.processingResult.translation}
          />
        )}
      </div>
      ...
      <div className="flex flex-wrap justify-end gap-2">
        <Button variant="outline" className="cursor-pointer" onClick={() => { navigator.clipboard.writeText(doc.processingResult?.summary || ""); toast.success("Copied to clipboard"); }}>
